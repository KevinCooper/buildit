...
def main(xmlin, prefix, keeplog):
  ...
  for c in commands:
    ...
    print "expected out %d got %d" % (len(expctout),len(out))
    print "expected err %s got %s" % (expcterr,err)
    print "expected err %d got %d" % (len(expcterr),len(err))
    print "expected return %d got %d" % (expctret,proc.returncode)
    if out != expctout or err != expcterr or proc.returncode != expctret:
      allpassed = False
  if allpassed == True:
    print "passed"
  else:
